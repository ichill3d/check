...
export default function AuthenticatedLayout({ header, children }) {
    ...
    const ownedTeams = auth.ownedTeams || [];
    const memberTeams = auth.memberTeams || [];
    const currentTeam = auth.currentTeam ?? null;
    ...
    const allTeams = (() => {
        ...
        ownedTeams.forEach(team => {
            if (!teamMap.has(team.id)) {
                teamMap.set(team.id, { ...team, type: 'owned' });
            }
        });

        memberTeams.forEach(team => {
            if (!teamMap.has(team.id)) {
                teamMap.set(team.id, { ...team, type: 'member' });
            }
        });
                    ...
                    <div className="flex h-16 justify-between">
                        <div className="flex">
                            ...
                            <div className="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                                <NavLink href={route('checklists.index')} active={route().current('checklists.index')}>
                                    Checklists
                                </NavLink>
                                <NavLink href={route('checklists.create')} active={route().current('checklists.create')}>
                                    New Checklist
                                </NavLink>
                            </div>
                        </div>
